...
most_recent_record <- function(entries, datetimes) {
  return(head(entries_since(entries, datetimes, 0)))
}
```
...
most_recent_entry <- most_recent_record(hill_data, hill_datetimes)
```
...
most_recent_entry <- most_recent_record(hunt_data, hunt_datetimes)
```
